...
import { JUMPING_TEXTS, JumpingLang } from './jumping-i18n';
...
export class JumpingComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  currentLang: JumpingLang = 'en';

  get t() {
    return JUMPING_TEXTS[this.currentLang];
  }
  ...
  private drawUI(): void {
    ...
    this.ctx.fillText(`${this.t.scoreLabel}: ${this.gameData.score}`, 10, 30);
    
    // 绘制高度
    this.ctx.textAlign = 'right';
    this.ctx.fillText(`${this.t.heightLabel}: ${Math.floor(this.gameData.height)}m`, width - 10, 30);
    ...
    if (this.gameData.gameMode === GameMode.SONG && this.gameData.currentSong) {
      ...
      this.ctx.fillText(`${this.t.progress}: ${progress}%`, width - 10, 100);
    } else if (this.gameData.gameMode === GameMode.CREATIVE) {
      this.ctx.font = '16px Arial';
      this.ctx.fillText(`🎹 ${this.t.creativeModeLabel}`, width - 10, 80);
      const sequenceLength = this.jumpingService.getCreativeSequence().length;
      this.ctx.fillText(`${this.t.sequenceLabel}: ${sequenceLength}`, width - 10, 100);
    }
    ...
    if (highScore > 0) {
      ...
      this.ctx.font = '16px Arial';
      this.ctx.fillText(`${this.t.highScore}: ${Math.floor(highScore)}m`, width / 2, 30);
    }
    ...
    if (this.gameData.state === GameState.PAUSED) {
      ...
      this.ctx.fillText(this.t.pauseLabel, width / 2, height / 2 - 20);
      
      this.ctx.font = '20px Arial';
      this.ctx.fillText(this.t.pressPContinue, width / 2, height / 2 + 20);
    }
  }
